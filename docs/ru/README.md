...
Помимо этого, пакет включает **`/srt`** — второй навык, который извлекает **субтитры SRT** из видео/аудио/URL с помощью распознавания речи Perso (speech-to-text), а затем поручает вашему агенту перевести их на любые нужные вам языки (либо просто выдаёт расшифровку на исходном языке как есть).

Под капотом он обращается к API Perso Dubbing, поэтому **необходим API-ключ Perso Dubbing** (один ключ действует для обоих навыков). → <a href="https://developers.perso.ai/api-keys" target="_blank" rel="noopener noreferrer">Получить API-ключ</a>
...
Скопируйте **обе** папки навыков в каталог навыков вашего хоста, рядом друг с другом (навык `srt` импортирует библиотеки навыка `dubbing` из соседней папки). Из корня репозитория:
...
После установки введите **`/dubbing`** в вашем агенте или просто скажите **«продублируй это видео»**, чтобы запустить его — либо **`/srt`** / **«сделай мне английские субтитры SRT для этого видео»** для субтитров. (Каждый способ установки выше устанавливает оба навыка.)
...
# Извлечь субтитры и поручить агенту перевести их (навык /srt)
npm run srt -- "clip.mp4" --target en,ja

# Только расшифровка — субтитры на исходном языке, без перевода
...
| Останавливается на середине (кончились кредиты, сбой, процесс завершён) | Прогресс на протяжении всего выполнения сохраняется в файл состояния (`*.dubresume.json` для `/dubbing`, `*.srtresume.json` для `/srt`). Выполните команду **`--resume "<state-file>"`**, указанную в уведомлении, чтобы завершить только оставшиеся части (уже готовые части пропускаются автоматически). |

---

## Конфиденциальность и телеметрия

`/dubbing` и `/srt` отправляют **анонимные** события использования, чтобы улучшать навыки — например, какое действие было выполнено (дубляж / синхронизация губ / разделение / извлечение субтитров), успешно ли оно завершилось, языковую пару, длительность медиафайла, версию приложения и ОС. Событие помечается только случайным идентификатором конкретной установки и никогда не включает ваш API-ключ, имена файлов или медиаконтент, учётную запись/email, а также идентификаторы рабочего пространства. Отказаться можно в любой момент с помощью переменной окружения `PERSO_NO_TELEMETRY`.
...
skills/dubbing/    Навык дубляжа (SKILL.md · lib/ · scripts/) — автономный
skills/srt/        Навык субтитров SRT (SKILL.md · scripts/) — использует lib/ из навыка dubbing
