...
Те же две команды `/plugin` работают и в **Claude Code CLI**.
...
| Команды `/plugin` или меню Plugins не реагируют | Вы находитесь в **облачной сессии** — плагины работают только в сессиях **Local** (и SSH). Переключите окружение на Local и повторите. |
